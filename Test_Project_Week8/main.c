...
int timeBetweenCalculations=25;		//Time in ms
float PI=3.1415926;
float wGyro = 10.0;
...
//	UARTSend(fabs(quadRoll));
	...
	pitchOffset = quadPitch - androidPitch;
	rollOffset = quadRoll - androidRoll;
	...
	if(recievedCommands[4] == 15){		//If in manual mode		change it to a 2 i put it at 3 to disable it basically
