 ...
 * ADC readings
 * Watchdog timers  this could be used incase of emergency
 * I2C coomunication
 * Something fun for the hell of it idk
 * I also need to buy a 3.3V regulator. Shit
...
#include "driverlib/ssi.h"		//Enables serial communication
...
short commandAddress=-1;			//Can not use byte they are unsigned by default
 ...
 *								PWM Functions
 *********************************************************************************************/
//This function will set the PWM for the desired timer and thus desired pin on off for duty cycle from 0-1000 note OFF not ON.
...
inline void PWMSet(unsigned long timer, unsigned long dutyLow){
	TimerMatchSet(timer,TIMER_A,dutyLow);
}
...
void Uart2IntHandler(void){
	...
	if(ulStatus & UART_INT_TX){
		...
		while(UARTCharsAvail(UART2_BASE)){
			...
			if((buffer == 0xAA)&&(commandAddress == -1)){
				commandAddress=0;	//Set the command Address to 0 this way the dummy byte is the only starting byte for a packet to be accepeted
			}else if(commandAddress>=0){						//Read in because it's not a dummy byte, prep for reading
				recievedCommands[commandAddress]=buffer;
				commandAddress++;
				commandAddress = (commandAddress>6) ? -1 : commandAddress;			//If greater than 6 set to -1 else set to self
			}
		}
	}
...
void I2CRead(unsigned char device, unsigned char regiester, unsigned char num, short buff[]){
	I2CMasterSlaveAddrSet(I2C1_MASTER_BASE, device, false);	//Set Device to transmit to
...
void rawAccToG(short * raw, float * RwAcc){
	float R;
	RwAcc[0]=((raw[1]<<8)|raw[0])/256;
...
void rawGyroToDegsec(short * raw, float * Gyro_ds){
	Gyro_ds[0]=(((raw[2]<<8)|raw[3])+g_offx)/14.375;
...
void main(void) {
	...
	SysCtlPeripheralEnable(SYSCTL_PERIPH_SSI0);		//Turn on Serial communication on serial 0
	...
	//Basic LED Out Test	Not sure why this is here look into
	...
	//This next line freezes the debugger. However, PWM is working on the first timer. I may have fixed it not sure
	...
	TimerMatchSet(TIMER0_BASE, TIMER_A, 1000);  //Duty cycle = (1-%desired)*1000 note this means this number is percent low not percent high
	TimerMatchSet(TIMER1_BASE, TIMER_A, 1000);
	TimerMatchSet(TIMER2_BASE, TIMER_A, 1000);
	TimerMatchSet(TIMER3_BASE, TIMER_A, 1000);
