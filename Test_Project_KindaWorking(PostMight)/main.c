...
short recievedCommands[5];			//For the same reason here I will only be reading chars in here BUT the char get function can return -1 which would overflow the char
...
void main(void) {
	...
	while(1){
		...
		//UARTSend(0xAA);
			...
			//integral_pitch += pitch_error*interval;
			//integral_roll += roll_error*interval;
			integral_pitch = 0;
			integral_roll = 0;

			derivative_pitch = Gyro_ds[0];
			derivative_roll = Gyro_ds[1];
			char kp_pitch = 0.7;
			char ki_pitch = 0;
			char kd_pitch = 0.2;
			char kp_roll = 1;
			char ki_roll = 0.006;
			char kd_roll = 0.15;
			final_pitchOffset = (kp_pitch*pitch_error)+(ki_pitch*integral_pitch)+(kd_pitch*derivative_pitch);
			final_rollOffset = (kp_roll*roll_error)+(ki_roll*integral_roll)+(kd_roll*derivative_roll);
